...
# from ohdsi.feature_extraction import get_db_covariate_data
...
class OhdsiTask(Task):
    ...
    def on_task_prerun(self, task_id, task, *args, **kwargs):
        ...
        connection_details = create_connection_details(
            dbms, server=f"{server}/{database}", user=user, password=password, port=port
        )
...
def background_task(self, a, b):
    ...
    with redirect_stdout(self.log_stream):
        try:
            result = query_sql(self.connection, "SELECT * FROM omopcdm.person LIMIT 3")
    ...
    return result.to_json(
        default_handler=str
    )  # {"data": result, "log": self.log_stream.getvalue()}
